...
const GOOGLE_SHEETS_WEBHOOK_URL =
  "https://script.google.com/macros/s/AKfycbzNQjI-dX7O4SZhj9X8v65U05gBqiFExWG60h90eVnenPc0OliMKqVk4f8LSFM0wt3LOg/exec";
...
const translations = {
  it: {
    ...
    notes: "Note al locale (es. richieste speciali, all'aperto)",
    notTonight: 'Annulla',
    ...
    <div className="modal-backdrop" style={{ backgroundColor: 'rgba(15, 23, 42, 0.95)', zIndex: 9999 }}>
      <div className="modal" style={{ maxWidth: '420px', textAlign: 'center', padding: '36px 24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <button className="primary-button" style={{ width: '100%', justifyContent: 'center', padding: '14px', fontSize: '16px' }} onClick={() => onSelect('it')}>
            Italiano
          </button>
          <button className="outline-button" style={{ width: '100%', justifyContent: 'center', padding: '14px', fontSize: '16px' }} onClick={() => onSelect('en')}>
            English
          </button>
        </div>
      </div>
        ...
        <nav className="nav-links" aria-label="Main navigation">
          {nav.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href} className={`nav-link ${location === href ? 'active' : ''}`}>
              <Icon size={14} /> {label}
              {href === '/events' && <span className="nav-count">{eventsCount}</span>}
            </Link>
          ))}
        </nav>
            ...
            <div className="user-menu-wrap">
              ...
              {menuOpen && (
                <div className="user-menu" role="menu">
                  <div className="user-menu-caption">{user.email}</div>
                  <Link href="/bookings" onClick={() => setMenuOpen(false)} role="menuitem"><Ticket size={14} /> {t.myBookings}</Link>
                  <button onClick={onSignOut} role="menuitem"><X size={14} /> {t.logout}</button>
                </div>
              )}
            </div>
          ) : (
...
function SignInModal({ onClose, onSignIn }: { onClose: () => void; onSignIn: () => void }) {
  return (
    <Modal onClose={onClose} label="Sign in to Spotti">
      <div className="modal-head">
        <div>
          <span className="eyebrow">Your night, saved</span>
          <h2>Keep the good plans.</h2>
          <p>Sign in to book tables and keep every reservation in one place.</p>
        </div>
        <button className="icon-button" onClick={onClose} aria-label="Close sign in"><X size={17} /></button>
      </div>
      <div className="modal-body">
        <div className="signin-box">
          <Sparkles size={22} />
          <h3>Continue with Google</h3>
          <p>We use your Google account to make bookings personal. No password to remember, no inbox noise.</p>
          <button className="primary-button" onClick={onSignIn}>Continue with Google</button>
        </div>
      </div>
    </Modal>
  );
...
function ConfirmDeleteModal({ lang, onClose, onConfirm }: { lang: Language; onClose: () => void; onConfirm: () => void }) {
  const t = translations[lang];
  return (
    <Modal onClose={onClose} label={t.confirmDeleteTitle}>
      <div className="modal-head">
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
      ...
      <div className="modal-body">
        <p style={{ margin: '12px 0 24px 0', color: '#64748b', fontSize: '14px' }}>
          {t.confirmDeleteText}
        </p>
...
function BookingModal({ venue, event, lang, onClose, onBooked }: { venue: Venue; event?: CityEvent; lang: Language; onClose: () => void; onBooked: (booking: Booking) => void }) {
  ...
  return (
    <Modal onClose={onClose} label={`Book ${venue.name}`}>
      <div className="modal-head">
        <div>
          <span className="eyebrow">{event ? t.reserveSpot : t.makePlan}</span>
          <h2>{event ? event.title : venue.name}</h2>
          <p>{venue.address} {event ? `${t.eventStart}: ${event.time}` : t.chooseTime}</p>
        </div>
        <button className="icon-button" onClick={onClose} aria-label="Close booking form"><X size={17} /></button>
      </div>
      <form className="modal-body" onSubmit={submit}>
        <hr className="modal-divider" />
        {errorMessage && (
          <div style={{ color: '#d32f2f', backgroundColor: '#fde8e8', padding: '10px 14px', borderRadius: '8px', marginBottom: '16px', fontSize: '13px', fontWeight: 500 }}>
            {errorMessage}
          </div>
        )}
        <div className="form-grid">
          <div className="field">
            ...
            <input id="booking-first-name" value={firstName} onChange={(e) => setFirstName(e.target.value)} required />
          </div>
          <div className="field">
            <label htmlFor="booking-last-name">{t.lastName}</label>
            <input id="booking-last-name" value={lastName} onChange={(e) => setLastName(e.target.value)} required />
          </div>
          <div className="field">
          ...
          <div className="field">
            <label htmlFor="booking-time">{venue.type === 'restaurant' ? t.arrivalTime : t.time}</label>
            <input id="booking-time" type="time" value={time} onChange={(e) => setTime(e.target.value)} required />
          </div>
          <div className="field full">
            <label htmlFor="booking-guests">{t.partySize}</label>
            <select id="booking-guests" value={guests} onChange={(e) => setGuests(e.target.value)}>
              ...
              <option>5 {t.guests}</option>
              <option>6 {t.guests}</option>
            </select>
          </div>
          <div className="field full">
        ...
        <div className="form-actions">
          <button type="button" className="outline-button" onClick={onClose}>{t.notTonight}</button>
          <button type="submit" className="primary-button"><Check size={15} /> {t.confirmBooking}</button>
        </div>
      </form>
    </Modal>
  );
...
function Explore({ venues, lang }: { venues: Venue[]; lang: Language }) {
  ...
  const filtered = useMemo(() => venues.filter((venue) =>
    (!activeFilters.length || activeFilters.includes(venue.type)) &&
    `${venue.name} ${venue.address}`.toLowerCase().includes(search.toLowerCase())
  ), [activeFilters, search, venues]);

  const filters = [
    { key: 'all' as const, label: t.allSpots, icon: Sparkles },
    { key: 'bar' as const, label: t.bars, icon: GlassWater },
    { key: 'restaurant' as const, label: t.restaurants, icon: Utensils },
    { key: 'club' as const, label: t.clubs, icon: Ticket }
  ];
  ...
  return (
    <main className="map-page">
      <MapContainer center={mapCenter} zoom={14} minZoom={11} maxZoom={18} zoomControl={false} className="map-canvas" scrollWheelZoom>
        ...
        {filtered.map((venue) => (
          <Marker key={venue.id} position={[venue.lat, venue.lng]} icon={venueIcon(venue, selected?.id === venue.id)} eventHandlers={{ click: () => setSelected(venue) }}>
            <Popup>
              <div className="leaflet-popup-content-inner">
                ...
                <a href={`https://www.google.com/maps/dir/?api=1&destination=${venue.lat},${venue.lng}`} target="_blank" rel="noreferrer">
                  <Navigation size={13} /> {t.takeMeThere}
                </a>
              </div>
            </Popup>
          </Marker>
        ...
        <div className="map-topbar">
          <label className="map-search">
            <Search size={17} color="#2b7468" />
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t.searchPlaceholder} aria-label="Search venues" />
            <span className="eyebrow" style={{ fontSize: 9 }}>Teramo</span>
          </label>
          <div className="map-note">
            <LocateFixed size={15} />
            <span>{t.showingIn} <strong>Teramo</strong></span>
          </div>
        </div>
        <div className="filter-rail">
          {filters.map(({ key, label, icon: Icon }) => {
            const active = key === 'all' ? !activeFilters.length : activeFilters.includes(key);
            return (
              <button
                key={key}
                className={`filter-chip ${active ? 'active' : ''}`}
                onClick={() => key === 'all' ? setActiveFilters([]) : setActiveFilters((current) => current.includes(key) ? current.filter((item) => item !== key) : [...current, key])}
              >
                <Icon size={13} />
                <span>{label}</span>
              </button>
        ...
        {!venues.length && (
          <div className="map-empty">
            <strong>{t.noVenues}</strong>
            <span>{t.noVenuesSub}</span>
          </div>
        )}
      </MapContainer>
      {selected && (
        <section className="map-panel">
          <div className="panel-image"><div className="panel-image-text">{selected.type}</div></div>
          <div className="panel-body">
            <div className="eyebrow">{selected.type}</div>
            <h2>{selected.name}</h2>
            <div className="panel-meta">
              <span className="meta-item"><MapPin size={12} /> {selected.address}</span>
              <span className="meta-item"><Clock3 size={12} /> {selected.hours}</span>
            </div>
            <div className="panel-actions">
              <button className="outline-button" onClick={() => setLocation(`/events?venue=${selected.id}`)}>
                <CalendarDays size={14} /> {t.seeEvents}
              </button>
              <a className="primary-button" href={`https://www.google.com/maps/dir/?api=1&destination=${selected.lat},${selected.lng}`} target="_blank" rel="noreferrer">
                <Navigation size={14} /> {t.takeMeThere}
              </a>
            </div>
          </div>
        </section>
  ...
  return (
    <main className="page">
      <div className="page-heading">
        <div>
          <span className="eyebrow">Spotti</span>
          <h1 className="page-title">{t.plansWithPulse}</h1>
          <p className="page-copy">{t.datedThings}</p>
        </div>
        <div className="heading-actions">
          <button className="soft-button" onClick={() => setSelectedDate(today)}>{t.today}</button>
          <Link href="/" className="outline-button"><MapPin size={14} /> {t.browseMap}</Link>
        </div>
      </div>
      {dates.length ? (
        <div className="date-strip">
          {dates.map((date) => {
            const parts = formatEventDate(date, lang);
            return (
              <button key={date} className={`date-button ${selectedDate === date ? 'active' : ''}`} onClick={() => setSelectedDate(date)}>
                <strong>{parts.day}</strong>
                <span>{parts.weekday} {parts.month}</span>
              </button>
            );
          })}
        </div>
        ...
        <div className="event-list">
          {shown.length ? (
            shown.map((event, index) => {
              ...
              const parts = formatEventDate(event.date, lang);
              return (
                <article className="event-card" key={event.id} style={{ animationDelay: `${index * 70}ms` }}>
                  <div className={`event-poster ${event.posterUrl ? '' : 'event-poster-empty'}`} style={event.posterUrl ? { backgroundImage: `url(${event.posterUrl})` } : undefined}>
      ...
      {eventDetail && (
        <Modal onClose={() => setEventDetail(null)} label={`Event details for ${eventDetail.title}`}>
          <div className="modal-head">
            <div>
          ...
          <div className="modal-body">
            <hr className="modal-divider" />
            <div className="panel-meta" style={{ marginBottom: 18 }}>
              <span className="meta-item"><MapPin size={13} /> {eventVenue(eventDetail)?.address ?? eventDetail.venueName}</span>
              <span className="meta-item"><Clock3 size={13} /> {eventDetail.time}</span>
            </div>
            {eventVenue(eventDetail) ? (
              <button className="primary-button" onClick={() => { const venue = eventVenue(eventDetail); if (venue) { setEventDetail(null); onBook(venue, eventDetail); } }}>
                <Ticket size={14} /> {t.bookThisEvent}
              </button>
            ) : (
              <p className="page-copy">{t.notInVenues}</p>
            )}
          </div>
        </Modal>
      )}
  ...
  return (
    <main className="page">
      <div className="page-heading">
        <div>
          <span className="eyebrow">Spotti</span>
          <h1 className="page-title">{t.plansInMotion}</h1>
          <p className="page-copy">{t.plansInMotionCopy}</p>
        </div>
      </div>
      {bookings.length ? (
        <div className="booking-grid">
          <div>
            {bookings.map((booking, index) => {
              ...
              const venueName = venue?.name ?? booking.venueName ?? 'Locale';
              const venueType = venue?.type;
              return (
                <article className="booking-card" key={booking.id} style={{ animationDelay: `${index * 70}ms` }}>
                  <div className="booking-badge">
                    {venueType === 'restaurant' ? <Utensils size={22} /> : venueType === 'club' ? <Ticket size={22} /> : <GlassWater size={22} />}
                  </div>
                  <div>
                    <span className="eyebrow">{venueType ?? 'booking'}</span>
                    <h3>{venueName}</h3>
                    <p>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span className="status-tag"><Check size={10} /> {booking.status}</span>
                    <button className="icon-button" onClick={() => onDeleteBooking(booking.id)} title={t.cancelBooking} aria-label={t.cancelBooking} style={{ color: '#d32f2f' }}>
                      <Trash2 size={16} />
                    </button>
                  </div>
                </article>
        ...
        <div className="empty-state">
          <div>
            <div className="empty-icon"><Ticket size={24} /></div>
            <h2>{t.noPlans}</h2>
            <p>{t.noPlansSub}</p>
            <button className="primary-button" onClick={onExplore}><Compass size={14} /> {t.exploreTonight}</button>
          </div>
...
function RouterContent({ user, venues, events, lang, onSignIn, onSignOut, onChangeLang, bookings, onBooking, onDeleteBooking }: { user: User | null; venues: Venue[]; events: CityEvent[]; lang: Language; onSignIn: () => void; onSignOut: () => void; onChangeLang: () => void; bookings: Booking[]; onBooking: (venue: Venue, event?: CityEvent) => void; onDeleteBooking: (bookingId: string) => void }) {
  ...
  return (
    <AppShell user={user} eventsCount={events.length} lang={lang} onSignIn={onSignIn} onSignOut={onSignOut} onChangeLang={onChangeLang}>
      <Switch>
        <Route path="/"><Explore venues={venues} lang={lang} /></Route>
        <Route path="/events"><EventsPage events={events} venues={venues} lang={lang} onBook={onBooking} /></Route>
        <Route path="/bookings"><BookingsPage bookings={bookings} venues={venues} lang={lang} onExplore={() => setLocation('/')} onDeleteBooking={onDeleteBooking} /></Route>
        <Route>
          <div className="page">
            <div className="empty-state">
              <div>
                ...
                <Link href="/" className="primary-button">Back to explore</Link>
              </div>
            </div>
          </div>
        </Route>
    ...
    const unsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        ...
        for (const item of snapshot.docs) {
          ...
          if (parsed) {
            const eventDateNormalized = parsed.date.slice(0, 10);
            if (eventDateNormalized < today) {
              try {
                await deleteDoc(doc(firestore, 'events', item.id));
                const qBookings = query(
                  collection(firestore, 'bookings'),
                  where('eventTitle', '==', parsed.title)
                );
                const bookingsSnap = await getDocs(qBookings);
                const deletePromises = bookingsSnap.docs.map((b) => {
                  const bData = b.data();
                  const bDate = toDateString(bData.date);
                  if (bDate < today || bDate === eventDateNormalized) {
                    return deleteDoc(doc(firestore, 'bookings', b.id));
                  }
    ...
    const unsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        ...
        for (const docSnap of snapshot.docs) {
          ...
          const bookingDate = toDateString(data.date);
          if (bookingDate && bookingDate < today) {
            try {
              await deleteDoc(doc(firestore, 'bookings', docSnap.id));
            } catch (err) {
              console.error('Errore eliminazione prenotazione scaduta:', err);
            }
          } else {
            activeBookings.push({
              id: docSnap.id,
              ...data,
              date: bookingDate || data.date,
            } as Booking);
          }
        }
