 ...
 return <div className="app-shell">
  <header className="app-nav">
   ...
   <nav className="nav-links" aria-label="Main navigation">
    {nav.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={`nav-link ${location === href ? 'active' : ''}`}><Icon size={14} /> {label}{href === '/events' && <span className="nav-count">{eventsCount}</span>}</Link>)}
   </nav>
   <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
    ...
    </div> : <button className="outline-button" onClick={onSignIn}><CircleUserRound size={15} /> {t.signIn}</button>}
   </div>
  </header>
  {children}
  <nav className="mobile-nav" aria-label="Mobile navigation">
   {nav.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={location === href ? 'active' : ''}><Icon size={16} /><span>{label}</span></Link>)}
  </nav>
 </div>;
}
...
function SignInModal({ onClose, onSignIn }: { onClose: () => void; onSignIn: () => void }) {
 return <Modal onClose={onClose} label="Sign in to Spotti">
  <div className="modal-head">
   <div><span className="eyebrow">Your night, saved</span><h2>Keep the good plans.</h2><p>Sign in to book tables and keep every reservation in one place.</p></div>
   <button className="icon-button" onClick={onClose} aria-label="Close sign in"><X size={17} /></button>
  </div>
  <div className="modal-body">
   <div className="signin-box"><Sparkles size={22} /><h3>Continue with Google</h3><p>We use your Google account to make bookings personal. No password to remember, no inbox noise.</p><button className="primary-button" onClick={onSignIn}>Continue with Google</button></div>
  </div>
 </Modal>;
}
...
function BookingModal({ venue, event, lang, onClose, onBooked }: { venue: Venue; event?: CityEvent; lang: Language; onClose: () => void; onBooked: (booking: Booking) => void }) {
 ...
 return <Modal onClose={onClose} label={`Book ${venue.name}`}>
  <div className="modal-head">
   <div><span className="eyebrow">{event ? t.reserveSpot : t.makePlan}</span><h2>{event ? event.title : venue.name}</h2><p>{venue.address} · {event ? `${t.eventStart}: ${event.time}` : t.chooseTime}</p></div>
   <button className="icon-button" onClick={onClose} aria-label="Close booking form"><X size={17} /></button>
  </div>
  <form className="modal-body" onSubmit={submit}>
   <hr className="modal-divider" />
   {errorMessage && <div style={{ color: '#d32f2f', backgroundColor: '#fde8e8', padding: '10px 14px', borderRadius: '8px', marginBottom: '16px', fontSize: '13px', fontWeight: 500 }}>{errorMessage}</div>}
   <div className="form-grid">
    ...
    <div className="field full"><label htmlFor="booking-guests">{t.partySize}</label>
     <select id="booking-guests" value={guests} onChange={(e) => setGuests(e.target.value)}>
      <option>1 {t.guest}</option><option>2 {t.guests}</option><option>3 {t.guests}</option><option>4 {t.guests}</option><option>5 {t.guests}</option><option>6 {t.guests}</option>
     </select>
    </div>
   ...
   <div className="form-actions"><button type="button" className="outline-button" onClick={onClose}>{t.notTonight}</button><button type="submit" className="primary-button"><Check size={15} /> {t.confirmBooking}</button></div>
  </form>
 </Modal>;
}
...
function EventsPage({ events, venues, lang, onBook }: { events: CityEvent[]; venues: Venue[]; lang: Language; onBook: (venue: Venue, event?: CityEvent) => void }) {
 ...
 return <main className="page">
  <div className="page-heading">
   <div><span className="eyebrow">Spotti</span><h1 className="page-title">{t.plansWithPulse}</h1><p className="page-copy">{t.datedThings}</p></div>
   <div className="heading-actions"><button className="soft-button" onClick={() => setSelectedDate(today)}>{t.today}</button><Link href="/" className="outline-button"><MapPin size={14} /> {t.browseMap}</Link></div>
  </div>
  {dates.length ? <div className="date-strip">{dates.map((date) => { const parts = formatEventDate(date, lang); return <button key={date} className={`date-button ${selectedDate === date ? 'active' : ''}`} onClick={() => setSelectedDate(date)}><strong>{parts.day}</strong><span>{parts.weekday} {parts.month}</span></button>; })}</div> : <div className="empty-state compact-empty"><div><div className="empty-icon"><CalendarDays size={24} /></div><h2>{t.noEvents}</h2><p>{t.noEventsSub}</p></div></div>}
  <div className="content-grid">
   <div className="event-list">{shown.length ? shown.map((event, index) => { const venue = eventVenue(event); const parts = formatEventDate(event.date, lang); return <article className="event-card" key={event.id} style={{ animationDelay: `${index * 70}ms` }}><div className={`event-poster ${event.posterUrl ? '' : 'event-poster-empty'}`} style={event.posterUrl ? { backgroundImage: `url(${event.posterUrl})` } : undefined}><span>{event.type}</span></div><div className="event-date"><strong>{parts.day}</strong><span>{parts.month}</span></div><div><span className="event-tag"><Sparkles size={10} /> {event.type}</span><h3>{event.title}</h3><p><strong>{event.venueName}</strong> · {event.time}{venue ? ` · ${venue.address}` : ''}</p></div><button className="primary-button" onClick={() => setEventDetail(event)}>{t.details}</button></article>; }) : <div className="empty-state"><div><div className="empty-icon"><CalendarDays size={24} /></div><h2>{t.quieterDate}</h2><p>{t.quieterDateSub}</p></div></div>}</div>
  ...
  {eventDetail && <Modal onClose={() => setEventDetail(null)} label={`Event details for ${eventDetail.title}`}><div className="modal-head"><div><span className="eyebrow">{formatEventDate(eventDetail.date, lang).weekday} {eventDetail.date}</span><h2>{eventDetail.title}</h2><p>{eventDetail.venueName} {eventDetail.time}</p></div><button className="icon-button" onClick={() => setEventDetail(null)} aria-label="Close event details"><X size={17} /></button></div><div className="modal-body"><hr className="modal-divider" /><div className="panel-meta" style={{ marginBottom: 18 }}><span className="meta-item"><MapPin size={13} /> {eventVenue(eventDetail)?.address ?? eventDetail.venueName}</span><span className="meta-item"><Clock3 size={13} /> {eventDetail.time}</span></div>{eventVenue(eventDetail) ? <button className="primary-button" onClick={() => { const venue = eventVenue(eventDetail); if (venue) { setEventDetail(null); onBook(venue, eventDetail); } }}><Ticket size={14} /> {t.bookThisEvent}</button> : <p className="page-copy">{t.notInVenues}</p>}</div></Modal>}
 </main>;
}
...
function BookingsPage({ bookings, venues, lang, onExplore, onDeleteBooking }: { bookings: Booking[]; venues: Venue[]; lang: Language; onExplore: () => void; onDeleteBooking: (bookingId: string) => void }) {
 ...
 return <main className="page">
  <div className="page-heading">
   <div><span className="eyebrow">Spotti</span><h1 className="page-title">{t.plansInMotion}</h1><p className="page-copy">{t.plansInMotionCopy}</p></div>
  </div>
  {bookings.length ? <div className="booking-grid">
   <div>{bookings.map((booking, index) => { const venue = venues.find((item) => item.id === booking.venueId); const venueName = venue?.name ?? booking.venueName ?? 'Locale'; const venueType = venue?.type; return <article className="booking-card" key={booking.id} style={{ animationDelay: `${index * 70}ms` }}><div className="booking-badge">{venueType === 'restaurant' ? <Utensils size={22} /> : venueType === 'club' ? <Ticket size={22} /> : <GlassWater size={22} />}</div><div><span className="eyebrow">{venueType ?? 'booking'}</span><h3>{venueName}</h3><p>{booking.date} · {booking.time}<br />{booking.guests}{venue?.address ? ` · ${venue.address}` : ''}{booking.eventTitle ? <><br />{booking.eventTitle}</> : null}{booking.notes ? <><br /><em>Note: {booking.notes}</em></> : null}</p></div><div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><span className="status-tag"><Check size={10} /> {booking.status}</span><button className="icon-button" onClick={() => onDeleteBooking(booking.id)} title={t.cancelBooking} aria-label={t.cancelBooking} style={{ color: '#d32f2f' }}><Trash2 size={16} /></button></div></article>; })}</div>
   <aside className="side-feature">
  ...
  </div> : <div className="empty-state"><div><div className="empty-icon"><Ticket size={24} /></div><h2>{t.noPlans}</h2><p>{t.noPlansSub}</p><button className="primary-button" onClick={onExplore}><Compass size={14} /> {t.exploreTonight}</button></div></div>}
 </main>;
}
...
function RouterContent({ user, venues, events, lang, onSignIn, onSignOut, onChangeLang, bookings, onBooking, onDeleteBooking }: { user: User | null; venues: Venue[]; events: CityEvent[]; lang: Language; onSignIn: () => void; onSignOut: () => void; onChangeLang: () => void; bookings: Booking[]; onBooking: (venue: Venue, event?: CityEvent) => void; onDeleteBooking: (bookingId: string) => void }) {
 const [, setLocation] = useLocation();
 return <AppShell user={user} eventsCount={events.length} lang={lang} onSignIn={onSignIn} onSignOut={onSignOut} onChangeLang={onChangeLang}>
  <Switch>
   ...
   <Route><div className="page"><div className="empty-state"><div><div className="empty-icon"><Compass size={24} /></div><h2>Page not found.</h2><Link href="/" className="primary-button">Back to explore</Link></div></div></div></Route>
  </Switch>
 </AppShell>;
}
